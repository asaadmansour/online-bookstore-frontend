...
export class VerifyEmailComponent implements OnInit {
    ...
    onSubmit(): void {
        ...
        const code = String(this.form.value.code || '').trim().toUpperCase();
    ...
    resend(): void {
        ...
        this.auth.resendVerification(this.email).subscribe({
            next: () => {
                ...
                this.success = 'New code sent! Check your inbox.';
                this.startCooldown();
            },
            error: (err) => {
                this.resendLoading = false;
                this.error = err?.error?.error || err?.message || 'Could not resend code';
            },
        });
    }
